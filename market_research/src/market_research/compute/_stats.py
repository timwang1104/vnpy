...
"""底层纯函数: rank_list / spearman / smoothed_share / daily_share / fwd_returns / ic_series / quintile_perf / total_ic.

搬自 examples/fundflow_viz/build_data.py，签名改为接收数据而非读 db。
数学逻辑零改动。
"""
...
def spearman(xs: Sequence[float], ys: Sequence[float]) -> float | None:
    ...
    if sxx == 0 or syy == 0:
        return None
    return sxy / ((sxx ** 0.5) * (syy ** 0.5))  # type: ignore[no-any-return]
