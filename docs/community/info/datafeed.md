...
### 通过第三方代理接入（15000 积分高权限 key）

本仓库自带 `vnpy_tushare` 实现，支持通过第三方代理（默认 `https://tt.xiaodefa.cn`）调用 tushare pro，使用 15000 积分高权限 key 时可获得日线/分钟线/日线指标/财务指标/合约信息/实时行情等广覆盖数据。代理地址可通过环境变量 `TUSHARE_BASE_URL` 或 `datafeed.username` 覆盖。
...
```json
{
    ...
    "datafeed.username": "https://fast.xiaodefa.cn",
    "datafeed.password": "<56位tushare token>"
}
```

字段约定：
- `datafeed.name`：固定为 `tushare`；
- `datafeed.username`：代理地址，留空则读环境变量 `TUSHARE_BASE_URL`，再留空则默认 `https://tt.xiaodefa.cn`；
- `datafeed.password`：tushare pro 56 位 token（**请勿提交到仓库**）。

交易所 → ts_code 后缀映射：

| VeighNa Exchange | ts_code 后缀 |
|---|---|
| SSE / SZSE / BSE | `.SH` / `.SZ` / `.BJ` |
| CFFEX | `.CFE` |
| SHFE | `.SHF` |
| CZCE | `.CZC` |
| DCE | `.DCE` |
...
K线频率映射（`Interval` → tushare `pro_bar(freq=...)`）：`MINUTE→min`、`HOUR→60min`、`DAILY→D`（走 `pro.daily`）、`WEEKLY→W`。

Token 读取顺序：VeighNa 集成路径下优先 `datafeed.password`，回退环境变量 `TUSHARE_API_KEY`；两者皆空时 `get_datafeed()` 返回 `BaseDatafeed` 兜底。脱离 UI 直接抓取请见 `examples/tushare_fetch/` 三个脚本（优先读环境变量 `TUSHARE_API_KEY`，再回退 `datafeed.password`）。
